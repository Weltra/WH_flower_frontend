      ...
      <div class="menu-container">
        <nav>
          <ul class="menu" role="list" style="list-style-type: none">
            ...
            <li>
              <router-link to="/Map"><a>赏花地图</a></router-link>
            </li>
            <li>
              <router-link to="/Feedback"><a>我的发现</a></router-link>
            </li>
            <li>
              <router-link to="/Route"><a>路线展示</a></router-link>
            </li>
            <li>
              <router-link to="/User"
                ><a style="font-size: 18px"
                  ><i class="el-icon-user-solid"></i></a
              ></router-link>
            </li>
          </ul>
        </nav>
      ...
      <ul class="search-result">
        <li v-for="item in items" :key="item.id" class="skeleton">
          ...
          <div class="skeleton__block">
            <h3>{{ item.name }}</h3>
            <p>
              <b><i class="el-icon-map-location"></i>位置：</b
              >{{ item.description }}
            </p>
          </div>
          <div class="skeleton__block">
            <p>
              <b><i class="el-icon-money"></i>门票价格：</b>{{ item.ticket }}
            </p>
            <p>
              <b><i class="el-icon-time"></i>开放时间：</b>{{ item.open_time }}
            </p>
            <p>
              <b><i class="el-icon-picture-outline-round"></i>花卉品种：</b
              >{{ item.flower_class }}
            </p>
          </div>
        </li>
...
export default {
  ...
  created() {
    this.$axios
      .get("http://127.0.0.1:8000/map_points/", {})
      .then((res) => {
  ...
  methods: {
    ...
    search() {
      ...
      this.$axios
        .post("http://127.0.0.1:8000/search_points/", this.keywords)
        .then((res) => {
          this.items = res.data.Points;
          this.$message({
            message: "搜索成功！",
